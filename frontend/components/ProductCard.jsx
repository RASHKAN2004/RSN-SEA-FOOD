  ...
  return (
    <div className="card group overflow-hidden">
      <Link href={`/products/${product.slug}`} className="block">
        <div className="relative h-48 w-full overflow-hidden bg-sea-50">
          ...
          <span className="absolute left-3 top-3 rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-semibold text-sea-700 shadow">
            {product.freshnessGrade || 'Daily Catch'}
          </span>
        </div>
      </Link>
      ...
      <div className="p-4">
        ...
        <p className="mt-1 text-xs text-gray-500">Minimum Quantity: {product.minimumQuantity}</p>
        <p className="mt-1 text-sm font-bold text-coral-600">Starting from {formatLKR(product.price)}</p>
