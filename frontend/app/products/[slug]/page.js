  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-10 lg:px-8">
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
        ...
        <div>
          ...
          <div className="mt-3 flex gap-2">
            {(product.gallery || [product.image]).map((img, i) => (
              <div
                key={i}
                className="relative h-16 w-16 overflow-hidden rounded-lg border border-sea-100"
              >
                <Image
                  src={img}
        ...
        {/* Details */}
        <div>
          <span className="rounded-full bg-sea-100 px-3 py-1 text-xs font-semibold text-sea-700">
          ...
          {/* Size selector */}
          <div className="mt-6">
            <p className="mb-2 text-sm font-semibold text-sea-900">
              Available Sizes
            </p>
                ...
                <button
                  ...
                  className={`rounded-full border px-4 py-1.5 text-sm font-medium transition ${
                    i === sizeIdx
                      ...
                      : "border-gray-300 text-gray-600 hover:border-sea-400"
                  }`}
                >
                  {s.label}
                </button>
          ...
          <p className="mt-1 text-2xl font-bold text-coral-600">
            {formatLKR(unitPrice)}{" "}
          ...
          <p className="mt-3 text-sm text-gray-500">
            Total:{" "}
            <span className="font-bold text-sea-900">{formatLKR(total)}</span>
          </p>
          ...
          <p className="mt-6 rounded-xl bg-sea-50 p-4 text-xs text-sea-700">
            Freshly sourced seafood, carefully handled and packed to maintain
            quality and freshness until delivery.
          </p>
        </div>
      </div>
    </section>
